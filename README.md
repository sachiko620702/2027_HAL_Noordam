...
- 固定重點：USJ 一天、USJ VIP 8 小時、好吃優先餐飲、精品購物、寶可夢／動漫、日本橋模型周邊、候補景點與候補美食資料庫

## 目前主控版本

- Version：V5.53.0 USJ VIP 8h Budget Update
- 主控資料源：GitHub main branch
- 更新目的：依使用者指示將 USJ Express Pass 7 改為 USJ VIP 8 小時，並重算 USJ、購物與主預算。
- 最新主預算：建議版 NT$272,454／兩人，節制版 NT$267,756／兩人，安全版 NT$280,000／兩人，硬上限 NT$280,000／兩人。
- USJ VIP 8 小時：暫以使用者提供金額 NT$87,000 納入主預算；需再次確認官方商品名稱、幣別、是否含 Studio Pass、餐飲包含範圍、可訂日與取消政策。
- USJ Studio Pass：仍保留 2027/10/10 購買目標，暫估兩人 NT$7,108；需確認 VIP 8 小時是否已包含。
- USJ 園區餐飲與購物：不再單獨列出；VIP 8 小時視為包含午餐、晚餐與園內餐飲；USJ 周邊購物與普通購物合併為 NT$14,000。
- Ambassador 續約 USD 250、精品包、珠寶、手錶另計。
- 住宿以低價那晚免費後的淨額 NT$65,900 估算，不把退房前臨時刷卡高峰值當成主預算。

## 目前已確認的新資訊

- 2026/11：買機票 EVA Air BR178／BR129，預算 TWD 36,000／兩人。
- 2026/12：續約洲際酒店大使 Ambassador，USD 250，先以外幣獨立追蹤，不硬轉 TWD。
- 2027/09/17：購買／預約 USJ VIP 8 小時，暫以 TWD 87,000 納入主預算；官方條件仍需再次確認。
- 2027/09/17：NOKA Roast & Grill 訂位，需確認套餐、主菜、90 分鐘暢飲、紀念日備註與特價菜單日不適用條件。
- 2027/10/10：買 KKday #129909 機場接送、KKday #536220 飯店 ↔ USJ 接送，以及 USJ Studio Pass；USJ Studio Pass 最終票價與是否已含於 VIP 8 小時仍需再次確認。
...
- Day 2 正式確認：USJ 一日遊，Express Pass 7 改為 USJ VIP 8 小時；VIP 8 小時視為包含午餐、晚餐與園內餐飲。
...
- 公開網站：`web/` 已於 V5.53.0 同步 USJ VIP 8 小時與預算更新；網站視覺沿用 V5.52.0 旅行社宣傳頁風格。
...
- [預約與購買時間軸 V5.53.0](docs/03_預約購買/2027大阪自由行_預約購買時間軸_V5.26.0.md)
...
- [預算模型](docs/06_預算/2027大阪自由行_預算模型_V5.26.0.md)
...
- [V5.53.0 USJ VIP 8 小時預算更新紀錄](docs/13_版本與更新/V5.53.0_USJ_VIP_8h_Budget_Update.md)

## 維護原則

1. main 分支是唯一主檔。
2. 新資訊先整理到 database，再同步更新 docs。
3. 不覆蓋既有決策；重大修改需更新 CHANGELOG 與 `docs/13_版本與更新/`。
